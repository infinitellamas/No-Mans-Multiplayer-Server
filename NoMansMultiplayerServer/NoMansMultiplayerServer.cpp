...
int main(int argc, char *argv[]){
	try
	{
		...
		std::cout << "---------------------------------" << std::endl;
		std::cout << "Initialising socket on tcp://127.0.0.1:5258" << std::endl;
		...
		while (GetAsyncKeyState(0x43) == 0) {
			...
			PlayerData current;
			bool parseResult = PlayerData::fromJSONString(&current, std::string(static_cast<char*>(incoming.data()), incoming.size()));
